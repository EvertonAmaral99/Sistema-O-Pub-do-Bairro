...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(!hasPermission(user,"REPORTS")&&!hasPermission(user,"MOVEMENT_MAINTENANCE")) return NextResponse.json({error:"Seu perfil não pode abrir esta venda."},{status:403});
  ...
  const sale=await query<{id:number;command_id:number;command_number:number|null;command_name:string|null;sale_channel:string;table_display:string;customer_id:number|null;customer_name:string|null;customer_cpf:string|null;subtotal_cents:number;discount_cents:number;service_fee_cents:number;total_cents:number;split_count:number;status:string;created_at:string;cancelled_at:string|null;user_name:string;delivery_id:number|null;delivery_status:string|null;pickup_code:string|null;courier_app_code:string|null;courier_app_code_not_required:boolean|null;collected_at:string|null}>(`SELECT s.id,s.command_id,c.command_number,c.command_name,c.sale_channel,cl.display_label AS table_display,s.customer_id,COALESCE(customer.name,c.customer_name) AS customer_name,customer.cpf AS customer_cpf,s.subtotal_cents,s.discount_cents,s.service_fee_cents,s.total_cents,s.split_count,s.status,s.created_at,s.cancelled_at,u.name AS user_name,d.id AS delivery_id,d.status AS delivery_status,d.pickup_code,d.courier_app_code,d.courier_app_code_not_required,d.collected_at FROM sales s JOIN commands c ON c.id=s.command_id JOIN command_locations cl ON cl.command_id=c.id JOIN users u ON u.id=s.created_by LEFT JOIN customers customer ON customer.id=s.customer_id LEFT JOIN delivery_orders d ON d.sale_id=s.id WHERE s.id=$1`,[saleId]);
  if(!sale.rows[0]) return NextResponse.json({error:"Venda não encontrada."},{status:404});
  const [items,payments]=await Promise.all([
    query<{id:number;product_name:string;quantity:string;unit_price_cents:number;display_unit:string}>("SELECT id,product_name,quantity::text,unit_price_cents,display_unit FROM order_items WHERE command_id=$1 AND (status<>'CANCELLED' OR ($2::timestamptz IS NOT NULL AND cancelled_at=$2::timestamptz)) ORDER BY id",[sale.rows[0].command_id,sale.rows[0].cancelled_at]),
