...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const [items,payments]=await Promise.all([
    query<{id:number;product_name:string;quantity:string;unit_price_cents:number;display_unit:string}>("SELECT id,product_name,quantity::text,unit_price_cents,display_unit FROM order_items WHERE sale_id=$1 ORDER BY id",[saleId]),
