...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const [items,payments]=await Promise.all([
    ...
    query<{id:number;method:string;amount_cents:number;staff_member_name:string|null;customer_name:string|null}>("SELECT p.id,p.method,p.amount_cents,p.staff_member_name,c.name AS customer_name FROM payments p LEFT JOIN customers c ON c.id=p.customer_id WHERE p.sale_id=$1 ORDER BY p.id",[saleId]),
  ]);
