...
export default async function SalePrintPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{formato?:string}>}){
  ...
  const sale=await query<{id:number;command_id:number;command_number:number|null;command_name:string|null;sale_channel:string;table_display:string;customer_name:string|null;subtotal_cents:number;discount_cents:number;service_fee_cents:number;total_cents:number;split_count:number;created_at:string;user_name:string;delivery_id:number|null;pickup_code:string|null;courier_app_name:string|null}>(`SELECT s.id,c.id AS command_id,c.command_number,c.command_name,c.sale_channel,cl.display_label AS table_display,COALESCE(customer.name,c.customer_name) AS customer_name,s.subtotal_cents,s.discount_cents,s.service_fee_cents,s.total_cents,s.split_count,s.created_at,u.name AS user_name,d.id AS delivery_id,d.pickup_code,d.courier_app_name
  ...
  return <main className={`print-page ${format==="80"?"receipt-80":format==="58"?"receipt-58":""}`}>
    ...
    <p>{quickSale?<><strong>Tipo:</strong> {data.delivery_id?"Retirada por aplicativo":"Venda rápida"}<br/></>:<><strong>Comanda:</strong> {commandLabel(data)}<br/><strong>Mesa(s):</strong> {data.table_display}<br/></>}<strong>Cliente:</strong> {data.customer_name||"Não informado"}<br/><strong>Atendente:</strong> {data.user_name}</p>{data.delivery_id&&<section className="print-delivery-code"><strong>CÓDIGO DE RETIRADA</strong><span>{data.pickup_code}</span><small>Informe estes 4 números ao cliente. O motoboy deverá dizer o mesmo código no balcão para retirar o pedido.</small>{data.courier_app_name&&<small>Serviço informado: {data.courier_app_name}</small>}</section>}<div className="divider"/>
