...
import { normalizePrintFormat } from "@/lib/print-format";

export const dynamic = "force-dynamic";

export default async function ShoppingListPrintPage({searchParams}:{searchParams:Promise<{formato?:string}>}) {
  await requirePermission("STOCK");
  const format=normalizePrintFormat((await searchParams).formato);
  ...
  return <main className={`print-page ${format==="58"?"receipt-58":""}`}><PrintActions/><header className="print-title"><BrandLogo className="print-logo"/><h1>O Pub do Bairro</h1><strong>LISTA DE COMPRAS PARA REPOSIÇÃO</strong><p>Gerada em {formatDateTime(new Date())}</p></header><div className="divider"/>
    {products.rows.length===0?<p style={{textAlign:"center"}}>Nenhum produto está no estoque mínimo.</p>:<table><thead><tr><th>Produto</th><th>Categoria</th><th>Saldo atual</th><th>Estoque mínimo</th><th>Comprar</th></tr></thead><tbody>{products.rows.map((product)=><tr key={product.id}><td><strong>{product.name}</strong></td><td>{product.category}</td><td>{formatQuantity(product.stock_quantity,product.sale_unit)}</td><td>{formatQuantity(product.min_stock,product.sale_unit)}</td><td style={{minWidth:format==="58"?50:180}}>________</td></tr>)}</tbody></table>}
