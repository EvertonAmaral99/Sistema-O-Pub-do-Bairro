...
export default async function KitchenPrintPage({ params, searchParams }: { params:Promise<{id:string}>; searchParams:Promise<{formato?:string}> }) {
  ...
  return <main className={`print-page ${format==="80"?"receipt-80":format==="58"?"receipt-58":""}`}><PrintActions/><header className="print-title"><h1>O Pub do Bairro</h1><strong>PEDIDO DE PRODUÇÃO</strong></header><div className="divider"/><p><strong>Comanda:</strong> #{data.command_number}<br/><strong>Mesa:</strong> {data.table_number}<br/><strong>Cliente:</strong> {data.customer_name||"Não informado"}<br/><strong>Enviado:</strong> {formatDateTime(data.created_at)}<br/><strong>Operador:</strong> {data.user_name}</p><div className="divider"/>{["KITCHEN","BAR"].map(area=>{const filtered=items.rows.filter(i=>i.destination===area); if(!filtered.length)return null; return <section key={area}><h2 style={{fontSize:18}}>{area==="KITCHEN"?"COZINHA":"BAR"}</h2>{filtered.map((item,index)=><div className="total-row" key={`${item.product_name}-${index}`} style={{fontSize:16,margin:"11px 0"}}><strong>{item.quantity}×</strong><strong style={{flex:1}}>{item.product_name}</strong></div>)}<div className="divider"/></section>;})}<p style={{textAlign:"center"}}>Ticket #{data.id}</p></main>;
}
