...
export default async function KitchenPrintPage({ params, searchParams }: { params:Promise<{id:string}>; searchParams:Promise<{formato?:string}> }) {
  await requireAnyPermission(["COMMANDS","KITCHEN","QUICK_SALES","DELIVERY"]); const ticketId=Number((await params).id); const format=normalizePrintFormat((await searchParams).formato);
