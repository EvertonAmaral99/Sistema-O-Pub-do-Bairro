...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  const products = await query<{ id:number; name:string; category:string; price_cents:number; stock_quantity:number|string; min_stock:number|string; destination:string; sale_unit:string; active:boolean }>("SELECT id,name,category,price_cents,stock_quantity,min_stock,destination,sale_unit,active FROM products ORDER BY active DESC,category,name");
  ...
  return <><div className="page-head"><div><p className="eyebrow">Cardápio</p><h2>Produtos e preços</h2><p>Cadastre os itens vendidos e indique o setor de preparo.</p></div></div>{erro && <div className="alert alert-error">{erro}</div>}
    <section className="card" style={{ marginBottom:22 }}><h3>Novo produto</h3><form action={createProductAction} className="form-grid">
      ...
      <div className="field"><label>Estoque inicial</label><input className="input" name="stock" type="number" min="0" step="0.001" defaultValue="0"/></div><div className="field"><label>Estoque mínimo</label><input className="input" name="minStock" type="number" min="0" step="0.001" defaultValue="0"/></div><div><button className="btn btn-primary" type="submit">Cadastrar produto</button></div>
    </form></section>
    <div className="table-wrap"><table><thead><tr><th>Produto</th><th>Categoria</th><th>Preço</th><th>Setor</th><th>Estoque</th><th>Situação</th></tr></thead><tbody>{products.rows.map((product)=><tr key={product.id}><td><strong>{product.name}</strong></td><td>{product.category}</td><td className="money">{formatMoney(product.price_cents)}</td><td>{area[product.destination]}</td><td className="number">{formatQuantity(product.stock_quantity,product.sale_unit)}</td><td><span className={`badge ${!product.active?"badge-gray":Number(product.stock_quantity)<=Number(product.min_stock)?"badge-red":"badge-green"}`}>{!product.active?"Inativo":Number(product.stock_quantity)<=Number(product.min_stock)?"Estoque baixo":"Ativo"}</span></td></tr>)}</tbody></table></div>
  </>;
}
