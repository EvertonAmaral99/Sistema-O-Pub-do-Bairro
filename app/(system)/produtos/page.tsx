...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  const products = await query<ProductRow>(`SELECT id,name,category,price_cents,stock_quantity,min_stock,destination,sale_unit,stock_per_sale_unit,active,
  ...
  return <>
    ...
    <section className="card" style={{ marginBottom:22 }}>
      ...
      <form action={createProductAction} className="form-grid">
        ...
        <div className="field"><label>Baixa de estoque por item vendido</label><input className="input" name="stockPerSaleUnit" type="number" min="0.001" step="0.001" placeholder="Ex.: 0,5"/><small>Para CHOPP 500ML controlado em litros, use 0,5. Se deixar vazio, o sistema reconhece o ML pelo nome.</small></div>
    ...
    <div className="table-wrap"><table><thead><tr><th>Foto</th><th>Produto</th><th>Categoria</th><th>Preço</th><th>Setor</th><th>Estoque</th><th>Baixa/venda</th><th>Mínimo</th><th>Situação</th><th>Ação</th></tr></thead><tbody>{products.rows.map((product)=><tr key={product.id}>
