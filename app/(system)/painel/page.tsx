...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  const [stats, commands] = await Promise.all([
    ...
    query<{ id: number; command_number: number; table_number: number; customer_name: string | null; opened_at: string; total: string }>(`SELECT c.id,c.command_number,t.number AS table_number,c.customer_name,c.opened_at,
      ...
      WHERE c.status='OPEN' GROUP BY c.id,t.number ORDER BY c.opened_at DESC LIMIT 8`),
  ]);
