...
type MovementRow={id:number;command_number:number|null;command_name:string|null;sale_channel:string;table_display:string;customer_name:string|null;customer_cpf:string|null;total_cents:number;status:string;created_at:string;payment_methods:string|null};

export default async function MovementMaintenancePage({searchParams}:{searchParams:Promise<{inicio?:string;fim?:string;pedido?:string;cliente?:string;erro?:string;sucesso?:string}>}){
  await requirePermission("MOVEMENT_MAINTENANCE");
  ...
  const [movements,customers,staffMembers]=await Promise.all([
    query<MovementRow>(`SELECT s.id,c.command_number,c.command_name,c.sale_channel,cl.display_label AS table_display,COALESCE(customer.name,c.customer_name) AS customer_name,customer.cpf AS customer_cpf,s.total_cents,s.status,s.created_at,
      (SELECT STRING_AGG(p.method,',' ORDER BY p.id) FROM payments p WHERE p.sale_id=s.id AND p.voided_at IS NULL) AS payment_methods
  ...
  return <>
    <div className="page-head"><div><p className="eyebrow">Gestão</p><h2>Manutenção de movimentos</h2><p>Localize uma venda para corrigir o cliente, reorganizar os pagamentos ou efetuar um cancelamento.</p></div><span className="badge badge-blue"><Wrench size={14}/> Acesso da gestão</span></div>
    {params.erro&&<div className="alert alert-error">{params.erro}</div>}
    {params.sucesso==="cancelada"&&<div className="alert alert-success">Venda #{params.pedido} cancelada e registrada no histórico.</div>}
    ...
    <section style={{marginTop:22}}><div className="page-head"><div><h3>Movimentos localizados</h3><p>{movements.rows.length} venda(s) encontrada(s). Clique no número para abrir a manutenção.</p></div></div>{movements.rows.length===0?<div className="card empty">Nenhuma venda encontrada com os filtros informados.</div>:<div className="table-wrap"><table><thead><tr><th>Pedido</th><th>Origem</th><th>Cliente</th><th>Data</th><th>Pagamentos</th><th>Total</th><th>Situação</th></tr></thead><tbody>{movements.rows.map((movement)=><tr key={movement.id}><td><MovementMaintenanceButton saleId={movement.id} customers={customers.rows} staffMembers={staffMembers.rows}/></td><td>{saleReferenceLabel(movement)}</td><td>{movement.customer_name||"Não vinculado"}{movement.customer_cpf&&<><br/><small>CPF {movement.customer_cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/,"$1.$2.$3-$4")}</small></>}</td><td>{formatDateTime(movement.created_at)}</td><td>{movement.payment_methods?.split(",").map(paymentMethodLabel).join(", ")||"—"}</td><td className="money">{formatMoney(movement.total_cents)}</td><td><span className={`badge ${movement.status==="COMPLETED"?"badge-green":"badge-red"}`}>{movement.status==="COMPLETED"?"Concluída":"Cancelada"}</span></td></tr>)}</tbody></table></div>}</section>
  </>;
}
