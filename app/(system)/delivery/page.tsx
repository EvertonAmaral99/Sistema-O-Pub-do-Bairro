...
import { Bike, CircleCheck, Clock3, KeyRound, PackageCheck, Plus, Printer, Save, ShieldCheck, Smartphone } from "lucide-react";
import { confirmDeliveryPickupAction,saveDeliveryAppCodeAction } from "@/app/system-actions";
import { ConfirmSubmitButton } from "@/components/confirm-submit-button";
import { LiveRefresh } from "@/components/live-refresh";
import { requireRole } from "@/lib/auth";
import { query } from "@/lib/db";
import { deliveryOrderLabel,deliveryStatusLabel,type DeliveryStatus } from "@/lib/delivery";
import { formatDateTime,formatMoney,formatQuantity } from "@/lib/format";

export const dynamic="force-dynamic";

type DeliveryRow={
  id:number;sale_id:number;command_id:number;pickup_code:string;courier_app_name:string|null;courier_app_code:string|null;status:DeliveryStatus;
...
export default async function DeliveryPage({searchParams}:{searchParams:Promise<{erro?:string;codigo?:string;retirada?:string;pedido?:string}>}){
  ...
  const [orders,items]=await Promise.all([
    query<DeliveryRow>(`SELECT d.id,d.sale_id,s.command_id,d.pickup_code,d.courier_app_name,d.courier_app_code,d.status,d.failed_attempts,
  ...
  return <>
    ...
    {params.codigo&&<div className="alert alert-success">Código do aplicativo salvo no pedido {params.pedido?deliveryOrderLabel(Number(params.pedido)):""}.</div>}
    ...
    {active.length===0?<div className="card empty delivery-empty"><Bike size={32}/><strong>Nenhum delivery aguardando retirada</strong><span>Selecione “Retirada por aplicativo” ao finalizar uma Venda rápida.</span></div>:<div className="delivery-grid">{active.map((order)=>{
      ...
      return <article className={`card delivery-card ${order.status.toLocaleLowerCase()} ${highlighted?"highlighted":""}`} key={order.id}>
        ...
        <form className="delivery-app-form" action={saveDeliveryAppCodeAction}><input type="hidden" name="deliveryId" value={order.id}/><div className="form-grid"><div className="field"><label><Smartphone size={14}/> Aplicativo ou serviço — opcional</label><input className="input" name="courierAppName" defaultValue={order.courier_app_name??""} maxLength={60} autoComplete="off" placeholder="Aplicativo informado pelo cliente"/></div><div className="field"><label>Código do aplicativo para o motoboy</label><input className="input" name="courierAppCode" defaultValue={order.courier_app_code??""} maxLength={40} autoComplete="off" placeholder="Informe o código recebido do cliente"/></div></div><button className="btn btn-light btn-small" type="submit"><Save size={15}/> Salvar código do aplicativo</button></form>
        <div className="delivery-pickup-panel"><div><ShieldCheck size={22}/><span><strong>Confirmação da retirada</strong><small>{order.status==="PREPARING"?"Aguarde todos os itens da cozinha ficarem prontos.":order.courier_app_code?"Peça ao motoboy o código de 4 dígitos e digite abaixo.":"Salve o código do aplicativo antes de liberar a retirada."}</small></span></div>{order.status==="READY"&&<form className="delivery-pickup-form" action={confirmDeliveryPickupAction}><input type="hidden" name="deliveryId" value={order.id}/><input className="input delivery-code-input" name="pickupCode" inputMode="numeric" pattern="[0-9]{4}" minLength={4} maxLength={4} autoComplete="off" placeholder="0000" aria-label="Código de retirada informado pelo motoboy" required/><ConfirmSubmitButton className="btn btn-primary" message={`Confirmar a retirada do pedido ${deliveryOrderLabel(Number(order.id))}?`} disabled={!order.courier_app_code||order.locked}><PackageCheck size={16}/> Confirmar e liberar</ConfirmSubmitButton></form>}{order.failed_attempts>0&&<small className="delivery-attempt-warning">{order.locked?"Confirmação bloqueada por 5 minutos após tentativas incorretas.":`${order.failed_attempts} tentativa(s) incorreta(s) recente(s).`}</small>}</div>
