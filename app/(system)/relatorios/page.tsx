...
import { SaleDetailsButton } from "@/components/sale-details-button";
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ erro?:string; inicio?:string; fim?:string }> }) {
  ...
  const [summary,sales,products,payments,currentCash]=await Promise.all([
    ...
    query<{ id:number; command_number:number|null; command_name:string|null;sale_channel:string; table_display:string; total_cents:number; status:string; user_name:string; created_at:string }>(`SELECT s.id,c.command_number,c.command_name,c.sale_channel,cl.display_label AS table_display,s.total_cents,s.status,u.name AS user_name,s.created_at FROM sales s JOIN commands c ON c.id=s.command_id JOIN command_locations cl ON cl.command_id=c.id JOIN users u ON u.id=s.created_by WHERE ${salesFilter} ORDER BY s.created_at DESC`,reportValues),
    query<{ product_name:string; quantity:string; total:string;display_unit:string }>(`SELECT oi.product_name,SUM(oi.quantity)::text AS quantity,SUM(oi.quantity*oi.unit_price_cents)::text AS total,oi.display_unit FROM order_items oi JOIN sales s ON s.id=oi.sale_id WHERE s.status='COMPLETED' AND ${salesFilter} GROUP BY oi.product_name,oi.display_unit ORDER BY SUM(oi.quantity) DESC,oi.product_name`,reportValues),
    query<{method:string;sales_count:string;total:string}>(`SELECT p.method,COUNT(DISTINCT p.sale_id)::text AS sales_count,COALESCE(SUM(p.amount_cents),0)::text AS total FROM payments p JOIN sales s ON s.id=p.sale_id WHERE s.status='COMPLETED' AND p.voided_at IS NULL AND ${salesFilter} GROUP BY p.method ORDER BY p.method`,reportValues),
  ...
  return <><div className="page-head"><div><p className="eyebrow">Desempenho</p><h2>Relatórios de vendas</h2><p>O resumo inicial acompanha o caixa aberto. Use as datas para consultar o histórico.</p></div>{currentCashMode&&openCash&&<span className="badge badge-green">Caixa #{openCash.id} em andamento</span>}</div>{params.erro&&<div className="alert alert-error">{params.erro}</div>}
    ...
    <section className="card report-payment-summary"><h3>Resumo por forma de pagamento</h3>{payments.rows.length===0?<div className="empty">Nenhum pagamento no intervalo exibido.</div>:<div className="table-wrap"><table><thead><tr><th>Forma</th><th>Quantidade de vendas</th><th>Total</th></tr></thead><tbody>{payments.rows.map((payment)=><tr key={payment.method}><td><strong>{paymentMethodLabel(payment.method)}</strong></td><td>{payment.sales_count}</td><td className="money">{formatMoney(payment.total)}</td></tr>)}</tbody></table></div>}</section>
    <div className="grid grid-2 report-sales-grid" style={{marginTop:22}}><section><h3>Vendas</h3><div className="table-wrap"><table><thead><tr><th>Venda</th><th>Origem</th><th>Data</th><th>Total</th><th>Situação</th></tr></thead><tbody>{sales.rows.map(s=><tr key={s.id}><td><SaleDetailsButton saleId={s.id}/><br/><small>{s.user_name}</small></td><td>{saleReferenceLabel(s)}</td><td>{formatDateTime(s.created_at)}</td><td className="money">{formatMoney(s.total_cents)}</td><td><span className={`badge ${s.status==="COMPLETED"?"badge-green":"badge-red"}`}>{s.status==="COMPLETED"?"Concluída":"Cancelada"}</span></td></tr>)}</tbody></table></div></section>
      <section><h3>Relatório completo de itens vendidos</h3>{products.rows.length===0?<div className="card empty">Nenhum item vendido no intervalo exibido.</div>:<div className="table-wrap"><table><thead><tr><th>Item</th><th>Quantidade</th><th>Total</th></tr></thead><tbody>{products.rows.map(p=><tr key={`${p.product_name}-${p.display_unit}`}><td><strong>{p.product_name}</strong></td><td>{formatQuantity(p.quantity,p.display_unit)}</td><td className="money">{formatMoney(p.total)}</td></tr>)}</tbody></table></div>}</section></div>
  </>;
}
