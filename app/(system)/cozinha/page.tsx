...
export default async function KitchenPage() {
  ...
  const items = await query<{ id:number; product_name:string; quantity:number|string;display_unit:string; status:string; command_number:number; table_display:string; sent_at:string;priority:boolean;priority_note:string|null }>(`SELECT oi.id,oi.product_name,oi.quantity,oi.display_unit,oi.status,c.command_number,cl.display_label AS table_display,oi.sent_at,c.priority,c.priority_note
