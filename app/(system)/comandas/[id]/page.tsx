...
import { CreditCard, Grid2X2, MinusCircle, Pencil, ReceiptText, Send, ShoppingCart, TriangleAlert, UtensilsCrossed, XCircle } from "lucide-react";
import { cancelCommandAction, removeItemAction, sendKitchenAction, updateCommandTablesAction } from "@/app/system-actions";
import { toggleCommandPriorityAction, updateCommandInfoAction } from "@/app/command-edit-actions";
...
export default async function CommandDetailPage({params,searchParams}:Params){
  ...
  const commandResult=await query<CommandRow>(`SELECT c.id,c.command_number,c.command_name,cl.display_label AS table_display,c.customer_name,c.opened_at,c.notes,c.status,c.priority
    FROM commands c JOIN command_locations cl ON cl.command_id=c.id WHERE c.id=$1`,[commandId]);
  ...
  const [items,products,tables,staffMembers,customers]=await Promise.all([
    ...
    query<{id:number;number:number;label:string;selected:boolean}>(`SELECT bt.id,bt.number,COALESCE(bt.label,'Mesa '||bt.number) AS label,
      ...
      FROM bar_tables bt
      WHERE bt.active=TRUE OR EXISTS(SELECT 1 FROM command_tables current_tables WHERE current_tables.command_id=$1 AND current_tables.table_id=bt.id)
      ORDER BY bt.number`,[commandId]),
  ...
  return <>
    <div className="page-head command-detail-page-head">
      <div>
        <p className="eyebrow">{command.table_display}</p>
        <h2>Comanda {commandLabel(command)}</h2>
        <p>{command.customer_name||"Cliente não informado"} · Aberta em {formatDateTime(command.opened_at)}</p>
      </div>
      <div className="actions command-detail-head-actions">
        {command.status==="OPEN"&&<form action={toggleCommandPriorityAction}>
          <input type="hidden" name="commandId" value={command.id}/>
          <button className={`command-priority-toggle ${command.priority?"active":""}`} type="submit" aria-pressed={command.priority} aria-label={command.priority?"Remover prioridade":"Marcar como prioridade"} title={command.priority?"Remover prioridade":"Marcar como prioridade"}>
    ...
    <div className="split-layout command-detail-layout">
      <section className="card command-products-panel">
        <div className="page-head" style={{marginBottom:14}}>
          <div><h3 style={{margin:0}}>Adicionar produtos</h3><p>Digite parte do nome para filtrar imediatamente.</p></div>
        </div>
      ...
      <aside className={`card sticky-card command-options-panel ${command.priority?"priority-alert":""}`}>
        ...
        {command.status==="OPEN"&&<section className="command-info-section">
          <div className="command-info-section-head">
            <div className="command-info-section-title"><Pencil size={17}/><div><h3>Dados da comanda</h3><p>Edite o nome e as informações do pedido mesmo depois de aberto.</p></div></div>
          </div>
          <details className="command-info-details">
            <summary><Pencil size={15}/> Editar informações do pedido</summary>
            <div className="command-info-details-body">
              <form action={updateCommandInfoAction} className="form-stack">
                <input type="hidden" name="commandId" value={command.id}/>
                <div className="form-grid command-edit-grid">
                  <div className="field"><label>Número da comanda</label><input className="input" name="commandNumber" type="number" min="1" step="1" defaultValue={command.command_number??""} placeholder="Opcional se houver nome"/></div>
                  <div className="field"><label>Nome da comanda</label><input className="input" name="commandName" maxLength={80} defaultValue={command.command_name??""} placeholder="Ex.: Aniversário João"/></div>
                </div>
                <div className="field"><label>Cliente / identificação</label><input className="input" name="customerName" maxLength={120} defaultValue={command.customer_name??""} placeholder="Opcional"/></div>
                <div className="field"><label>Observações</label><textarea className="textarea" name="notes" maxLength={1000} defaultValue={command.notes??""} rows={3} placeholder="Opcional"/></div>
                <small className="command-info-muted">A comanda precisa continuar com pelo menos um número ou um nome.</small>
                <button className="btn btn-primary btn-small" type="submit">Salvar informações</button>
              </form>
            </div>
          </details>
        </section>}
        ...
        {command.status==="OPEN"&&<section className="command-info-section">
          ...
          <details className="command-info-details">
            ...
            <div className="command-info-details-body">
              <form action={updateCommandTablesAction} className="form-stack">
                <input type="hidden" name="commandId" value={command.id}/>
                <div className="table-choice-grid command-table-choice-grid">{tables.rows.map((table)=><label className="table-choice" key={table.id}><input type="checkbox" name="tableIds" value={table.id} defaultChecked={table.selected}/><span><strong>{table.label}</strong><small>Mesa {table.number}</small></span></label>)}</div>
                <button className="btn btn-primary btn-small" type="submit">Salvar mesas da comanda</button>
              </form>
            </div>
          </details>
        </section>}
        ...
        {command.status==="OPEN"&&<section className="command-info-section">
          ...
          <PrintActionForm action={sendKitchenAction} className="form-stack">
            ...
            <div className="field"><label>Formato do pedido</label><select className="select" name="format" defaultValue="58"><option value="58">Térmica 58 mm</option><option value="a4">Folha A4</option></select></div>
