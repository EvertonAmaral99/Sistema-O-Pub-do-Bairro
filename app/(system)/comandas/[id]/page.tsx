        ...
        <section className="card">
          ...
          <div className="product-list">
            {products.rows.map((product) => { const converted=Math.abs(Number(product.stock_per_sale_unit)-1)>0.0001; const fractional=!converted&&["KG","L"].includes(product.sale_unit); return <form action={addItemAction} className="product-button product-add-card" key={product.id}>
              ...
              <strong>{product.name}</strong><small>{product.category} · Estoque {product.stock_unlimited?"Ilimitado":formatQuantity(product.stock_quantity,product.sale_unit)}{product.stock_shared?" · Compartilhado":""}</small><div className="money" style={{ marginTop: 10 }}>{formatMoney(product.price_cents)}</div>
