...
export default async function QuickSalePage({searchParams}:{searchParams:Promise<{rascunho?:string;nova?:string}>}){
  ...
  return <>
    ...
    <QuickSaleWorkspace key={forceNew?"new":initialDraft?`draft-${initialDraft.id}`:"default"} products={products.rows} staffMembers={staffMembers.rows} customers={customers.rows} cashOpen={cashOpen} userId={user.id} initialDraft={initialDraft} forceNew={forceNew}/>
  </>;
}
