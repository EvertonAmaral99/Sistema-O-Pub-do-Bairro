...
export function DashboardCommandCard({ command, canViewFinance }: { command:Command; canViewFinance:boolean }) {
  ...
  return <article className={`command-card dashboard-command-card ${command.priority?"priority-alert":""}`}>
    <button className={`dashboard-priority-shortcut ${command.priority?"active":""}`} type="button" onClick={()=>dialogRef.current?.showModal()} title={command.priority?"Alterar prioridade":"Marcar como prioridade"} aria-label={command.priority?`Alterar prioridade da comanda ${command.command_number}`:`Marcar comanda ${command.command_number} como prioridade`}><AlertTriangle size={17}/></button>
    ...
    <dialog className="priority-dialog" ref={dialogRef}>
      <div className="priority-dialog-head"><div><span className="eyebrow">Atalho de prioridade</span><h3>Comanda #{command.command_number}</h3><p>{command.table_display}</p></div><form method="dialog"><button className="priority-dialog-close" type="submit" aria-label="Fechar"><X size={19}/></button></form></div>
