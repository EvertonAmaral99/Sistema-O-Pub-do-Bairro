...
type SplitPayment={method:PaymentMethod|"";amount:string;staffMemberName:string};
type CustomerCredit={id:number;name:string;cpf:string;contact:string;balanceCents:number};
type PaymentAllocation={method:PaymentMethod;amountCents:number;staffMemberName?:string;customerId?:number};
...
export function PaymentForm({commandId,subtotal}:{commandId:number;subtotal:number}){
  ...
  const [remainderStaffName,setRemainderStaffName]=useState("");

  const paymentMethods=useMemo(()=>customerCredit&&customerCredit.balanceCents>0?[...basePaymentMethods,{value:"STORE_CREDIT" as const,label:`Crédito em loja — ${brl(customerCredit.balanceCents)}`}]:basePaymentMethods,[customerCredit]);
  const calculation=useMemo(()=>{
    ...
    if(people===1&&paymentMethod){
      if(paymentMethod==="STORE_CREDIT"){
        ...
        if(remainder>0&&remainderMethod) allocations.push({method:remainderMethod,amountCents:remainder,staffMemberName:remainderMethod==="STAFF_VOUCHER"?remainderStaffName:undefined});
      }else allocations.push({method:paymentMethod,amountCents:total,staffMemberName:paymentMethod==="STAFF_VOUCHER"?staffMemberName:undefined});
    }
    if(people>1){
      for(const payment of splitPayments.slice(0,people)){
        const amountCents=toCents(payment.amount);
        if(payment.method&&amountCents>0) allocations.push({method:payment.method,amountCents,staffMemberName:payment.method==="STAFF_VOUCHER"?payment.staffMemberName:undefined,customerId:payment.method==="STORE_CREDIT"?customerCredit?.id:undefined});
      }
    }
  ...
  },[customerCredit,discount,paymentMethod,remainderMethod,remainderStaffName,service,splitCount,splitPayments,staffMemberName,storeCreditAmount,subtotal]);
  ...
  const referencesComplete=calculation.allocations.every((payment)=>payment.method!=="STAFF_VOUCHER"||Boolean(payment.staffMemberName?.trim()));
  ...
  function updateSplitPayment(index:number,patch:Partial<SplitPayment>){
    setSplitPayments((current)=>{const next=Array.from({length:Math.max(current.length,index+1)},(_,rowIndex)=>current[rowIndex]||{method:"",amount:"",staffMemberName:""});next[index]={...next[index],...patch};return next;});
  }
