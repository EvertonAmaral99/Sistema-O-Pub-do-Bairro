...
import { CircleCheck, CircleDollarSign, CreditCard, Printer, Search, UserPlus, Users, X } from "lucide-react";
...
export function PaymentForm({commandId,subtotal,staffMembers,customers}:{commandId:number;subtotal:number;staffMembers:StaffMemberOption[];customers:CustomerOption[]}){
  ...
  return <PrintActionForm action={closeCommandAction} className="form-stack">
    <input type="hidden" name="commandId" value={commandId}/><input type="hidden" name="paymentAllocations" value={JSON.stringify(calculation.allocations)}/><input type="hidden" name="customerId" value={selectedCustomerId}/><input type="hidden" name="createCustomer" value={newCustomerOpen?"1":"0"}/>
