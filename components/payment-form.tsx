...
import { Bike, CircleCheck, CircleDollarSign, CirclePlus, CreditCard, KeyRound, Printer, Search, Smartphone, Store, Trash2, UserPlus, Users, X } from "lucide-react";
...
export function PaymentForm({commandId,subtotal,staffMembers,customers,mode="COMMAND",quickSaleItems="[]",quickSaleDraftId=null,initialQuickSaleDraft,onQuickSaleDraftChange,quickSaleSubmitAction,onSuccess,canSubmit=true}:{commandId?:number;subtotal:number;staffMembers:StaffMemberOption[];customers:CustomerOption[];mode?:"COMMAND"|"QUICK_SALE";quickSaleItems?:string;quickSaleDraftId?:number|null;initialQuickSaleDraft?:QuickSaleCheckoutDraft;onQuickSaleDraftChange?:(draft:QuickSaleCheckoutDraft)=>void;quickSaleSubmitAction?:(formData:FormData)=>Promise<{url?:string;error?:string}>;onSuccess?:()=>void;canSubmit?:boolean}){
  ...
  const [courierAppName,setCourierAppName]=useState(initialDraft.courierAppName);
  ...
  const quickSaleCheckoutDraft=useMemo<QuickSaleCheckoutDraft>(()=>({
    ...
    storeCreditAmount,remainderMethod,remainderStaffMemberId,format,fulfillmentType,courierAppName,courierAppCode,
  }),[courierAppCode,courierAppName,customerSearch,discount,format,fulfillmentType,newCustomerContact,newCustomerCpf,newCustomerName,newCustomerOpen,paymentMethod,paymentMode,remainderMethod,remainderStaffMemberId,selectedCustomerId,service,splitCount,splitPayments,staffMemberId,storeCreditAmount]);
  ...
  return <PrintActionForm action={mode==="QUICK_SALE"?(quickSaleSubmitAction??quickSaleAction):closeCommandAction} className="form-stack" onSuccess={onSuccess}>
    {mode==="COMMAND"?<input type="hidden" name="commandId" value={commandId}/>:<><input type="hidden" name="quickSaleItems" value={quickSaleItems}/><input type="hidden" name="quickSaleDraftId" value={quickSaleDraftId??""}/><input type="hidden" name="guestCustomerName" value={guestCustomerName}/><input type="hidden" name="fulfillmentType" value={fulfillmentType}/><input type="hidden" name="courierAppName" value={courierAppName}/><input type="hidden" name="courierAppCode" value={courierAppCode}/></>}<input type="hidden" name="paymentAllocations" value={JSON.stringify(calculation.allocations)}/><input type="hidden" name="splitCount" value={paymentMode==="MIXED"?"1":splitCount}/><input type="hidden" name="customerId" value={selectedCustomerId}/><input type="hidden" name="createCustomer" value={newCustomerOpen?"1":"0"}/>
