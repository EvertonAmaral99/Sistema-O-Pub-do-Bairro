...
import { Bike, CircleCheck, CircleDollarSign, CirclePlus, CreditCard, KeyRound, Printer, Search, Store, Trash2, UserPlus, Users, X } from "lucide-react";
...
import { BASE_PAYMENT_METHOD_OPTIONS, type BasePaymentMethod } from "@/lib/payments";
...
export function PaymentForm({commandId,subtotal,staffMembers,customers,mode="COMMAND",quickSaleItems="[]",quickSaleDraftId=null,initialQuickSaleDraft,onQuickSaleDraftChange,quickSaleSubmitAction,onSuccess,canSubmit=true}:{commandId?:number;subtotal:number;staffMembers:StaffMemberOption[];customers:CustomerOption[];mode?:"COMMAND"|"QUICK_SALE";quickSaleItems?:string;quickSaleDraftId?:number|null;initialQuickSaleDraft?:QuickSaleCheckoutDraft;onQuickSaleDraftChange?:(draft:QuickSaleCheckoutDraft)=>void;quickSaleSubmitAction?:(formData:FormData)=>Promise<{url?:string;error?:string}>;onSuccess?:()=>void;canSubmit?:boolean}){
  ...
  const [remainderMethod,setRemainderMethod]=useState<BasePaymentMethod|"">(initialDraft.remainderMethod);
  const [remainderStaffMemberId,setRemainderStaffMemberId]=useState(initialDraft.remainderStaffMemberId);
  const [format,setFormat]=useState<"58"|"a4">(initialDraft.format==="a4"?"a4":"58");
  const [fulfillmentType,setFulfillmentType]=useState(initialDraft.fulfillmentType);
  const [courierAppCode,setCourierAppCode]=useState(initialDraft.courierAppCode);
  ...
  const availableBasePaymentMethods=BASE_PAYMENT_METHOD_OPTIONS;
  ...
  const quickSaleCheckoutDraft=useMemo<QuickSaleCheckoutDraft>(()=>({
    ...
    storeCreditAmount,remainderMethod,remainderStaffMemberId,format,fulfillmentType,courierAppCode,
  }),[courierAppCode,customerSearch,discount,format,fulfillmentType,newCustomerContact,newCustomerCpf,newCustomerName,newCustomerOpen,paymentMethod,paymentMode,remainderMethod,remainderStaffMemberId,selectedCustomerId,service,splitCount,splitPayments,staffMemberId,storeCreditAmount]);
  ...
  const guestCustomerName=mode==="QUICK_SALE"&&!selectedCustomer&&!newCustomerOpen?customerSearch.trim().replace(/\s+/g," "):"";
  const guestCustomerNameValid=guestCustomerName===""||(guestCustomerName.length>=2&&guestCustomerName.length<=120);
  const customerIdentificationValid=newCustomerOpen?newCustomerComplete:Boolean(selectedCustomer)||customerSearch.trim()===""||(mode==="QUICK_SALE"&&guestCustomerNameValid);
  ...
  return <PrintActionForm action={mode==="QUICK_SALE"?(quickSaleSubmitAction??quickSaleAction):closeCommandAction} className="form-stack" onSuccess={onSuccess}>
    {mode==="COMMAND"?<input type="hidden" name="commandId" value={commandId}/>:<><input type="hidden" name="quickSaleItems" value={quickSaleItems}/><input type="hidden" name="quickSaleDraftId" value={quickSaleDraftId??""}/><input type="hidden" name="guestCustomerName" value={guestCustomerName}/><input type="hidden" name="fulfillmentType" value={fulfillmentType}/><input type="hidden" name="courierAppCode" value={courierAppCode}/></>}<input type="hidden" name="paymentAllocations" value={JSON.stringify(calculation.allocations)}/><input type="hidden" name="splitCount" value={paymentMode==="MIXED"?"1":splitCount}/><input type="hidden" name="customerId" value={selectedCustomerId}/><input type="hidden" name="createCustomer" value={newCustomerOpen?"1":"0"}/>
    ...
    <section className="card customer-identification"><div className="customer-identification-head"><div><span className="label">IDENTIFICAÇÃO DO CLIENTE — OPCIONAL</span><p>{mode==="QUICK_SALE"?"Digite um nome sem cadastro ou selecione um cliente cadastrado pelo nome ou CPF.":"Vincule nome ou CPF para localizar esta venda mais facilmente na manutenção de movimentos."}</p></div><button className="btn btn-light btn-small" type="button" onClick={toggleNewCustomer}><UserPlus size={15}/> {newCustomerOpen?"Voltar à busca":"Cadastrar novo"}</button></div>
      {!newCustomerOpen?<div className="customer-search-wrap"><div className="field"><label htmlFor="sale-customer-search">Nome ou CPF</label><div className="customer-search-input"><Search size={16}/><input id="sale-customer-search" className="input" value={customerSearch} onFocus={()=>setCustomerResultsOpen(true)} onChange={(event)=>{setCustomerSearch(event.target.value);setSelectedCustomerId("");setCustomerResultsOpen(true);}} maxLength={120} autoComplete="off" placeholder="Digite ao menos 2 caracteres"/>{customerSearch&&<button type="button" onClick={clearCustomerSelection} aria-label="Limpar cliente"><X size={15}/></button>}</div></div>
        ...
        {!selectedCustomer&&customerSearch.trim()!==""&&<small className="customer-selection-help">{mode==="QUICK_SALE"?(guestCustomerName.length>=2?`“${guestCustomerName}” aparecerá na notinha sem criar cadastro. Para vincular um cadastro, selecione um resultado acima.`:"Digite ao menos 2 caracteres para usar o nome na notinha."):"Selecione um resultado, cadastre o cliente ou limpe o campo para continuar sem identificação."}</small>}</div>:<div className="form-grid customer-quick-create"><div className="field"><label>Nome</label><input className="input" name="newCustomerName" value={newCustomerName} onChange={(event)=>setNewCustomerName(event.target.value)} minLength={2} maxLength={120} autoComplete="name" required/></div><div className="field"><label>CPF</label><input className="input" name="newCustomerCpf" value={newCustomerCpf} onChange={(event)=>setNewCustomerCpf(event.target.value)} inputMode="numeric" maxLength={14} placeholder="000.000.000-00" required/></div><div className="field"><label>Contato</label><input className="input" name="newCustomerContact" value={newCustomerContact} onChange={(event)=>setNewCustomerContact(event.target.value)} minLength={5} maxLength={120} autoComplete="tel" placeholder="Telefone ou WhatsApp" required/></div><small className="customer-quick-create-help">O cadastro será criado e vinculado automaticamente ao finalizar. Para não identificar o cliente, volte à busca e deixe o campo vazio.</small></div>}
    </section>
    {mode==="QUICK_SALE"&&<><p className="label payment-label">TIPO DE ATENDIMENTO</p><section className="card fulfillment-card"><div className="fulfillment-options" role="group" aria-label="Tipo de atendimento"><button className={`fulfillment-option ${fulfillmentType==="COUNTER"?"active":""}`} type="button" aria-pressed={fulfillmentType==="COUNTER"} onClick={()=>setFulfillmentType("COUNTER")}><Store size={20}/><span><strong>Balcão</strong><small>Cliente recebe o pedido no atendimento normal.</small></span></button><button className={`fulfillment-option ${fulfillmentType==="APP_PICKUP"?"active":""}`} type="button" aria-pressed={fulfillmentType==="APP_PICKUP"} onClick={()=>setFulfillmentType("APP_PICKUP")}><Bike size={20}/><span><strong>Retirada por aplicativo</strong><small>O cliente enviará o próprio motoboy.</small></span></button></div>{fulfillmentType==="APP_PICKUP"&&<div className="delivery-setup"><div className="delivery-setup-head"><KeyRound size={19}/><div><strong>O código de retirada será criado ao finalizar</strong><small>Informe os 4 dígitos ao cliente. O motoboy precisará dizer o mesmo código no balcão.</small></div></div><div className="field"><label>Código do aplicativo para o motoboy</label><input className="input" value={courierAppCode} onChange={(event)=>setCourierAppCode(event.target.value)} maxLength={40} autoComplete="off" placeholder="Preencha se o cliente já informou"/></div><small>Se ainda não houver código, deixe em branco. Na aba Delivery será possível informar o código ou marcar explicitamente “Sem código”.</small></div>}</section></>}
    ...
    <div className="field"><label>Formato da notinha</label><select className="select" name="format" value={format} onChange={(event)=>setFormat(event.target.value as "58"|"a4")}><option value="58">Térmica 58 mm</option><option value="a4">Folha A4</option></select></div>
    <button className="btn btn-primary" type="submit" disabled={!canSubmit||!paymentComplete||!customerIdentificationValid}><Printer size={16}/> {mode==="QUICK_SALE"?(fulfillmentType==="APP_PICKUP"?"Finalizar e gerar código de retirada":"Finalizar venda rápida e abrir notinha"):"Finalizar e abrir notinha"}</button>
  </PrintActionForm>;
}
